...
def _write_lines(path: Path, raw_lines: list[str]) -> dict:
    ...
    with open(path, "w", encoding="utf-8") as f:
        for line in normalised: f.write(line + "\n")
    
    changed = sum(1 for r in single_lines if len(r.strip()) > 10 and r.strip() != normalize_prompt(r))
    print(f"    Saved {len(normalised):,} prompts → {path.name}  "
...
def _line_count(path: Path) -> int:
    ...
    with open(path, "r", encoding="utf-8") as f: return sum(1 for ln in f if ln.strip())
...
def download_advbench() -> None:
    print("\n[advbench]")
    if _already_exists(SRC["advbench"]): return
    import requests, pandas as pd
    r = requests.get("https://raw.githubusercontent.com/llm-attacks/llm-attacks/main/data/advbench/harmful_behaviors.csv")
...
def download_harmbench() -> None:
    print("\n[harmbench]")
    if _already_exists(SRC["harmbench"]): return
    ...
    _DATA_CARD["harmbench"] = {**SOURCE_META["harmbench"], **_write_lines(SRC["harmbench"], prompts)}

def download_jailbreakbench() -> None:
    print("\n[jailbreakbench]")
    if _already_exists(SRC["jailbreakbench"]): return
    from datasets import load_dataset
    ds = load_dataset("JailbreakBench/JBB-Behaviors", "behaviors", split="harmful", token=True)
    data = cast(Dict[str, List[Any]], ds.to_dict())
    prompts =[str(x) for x in data["Goal"] if x]
    _DATA_CARD["jailbreakbench"] = {**SOURCE_META["jailbreakbench"], **_write_lines(SRC["jailbreakbench"], prompts)}

def download_xstest() -> None:
    print("\n[xstest]")
    if _already_exists(SRC["xstest"]): return
    import requests, pandas as pd
    r = requests.get("https://raw.githubusercontent.com/paul-rottger/xstest/main/xstest_prompts.csv")
    df = pd.read_csv(StringIO(r.text))
...
def download_alpaca(n_total: int, seed: int) -> None:
    print(f"\n[alpaca] (n={n_total})")
    if _is_sufficient(SRC["alpaca"], n_total): return
    from datasets import load_dataset
    ds = load_dataset("yahma/alpaca-cleaned", split="train")
    data = cast(Dict[str, List[Any]], ds.to_dict())
    candidates =[str(i).strip() for i, p in zip(data["instruction"], data["input"]) if not str(p).strip() and "\n" not in str(i).strip() and 20 < len(str(i).strip()) < 300]
    random.seed(seed)
...
def compose_splits(seed: int) -> dict:
    ...
    def sample_and_write(source_key: str, dest_name: str, n: int | None = None, disjoint_from: list[str] | None = None) -> list[str]:
    ...
    return {"timestamp": datetime.now().isoformat(), "seed": seed, "type": "OOD Generalization Split"}
...
def main() -> None:
    ...
    DATA_CARD_OUT.write_text(json.dumps({"generated": datetime.now().isoformat(), "sources": _DATA_CARD, "recipe": prov}, indent=2))
